...
def train_models_worker(chunk_with_id):
    ...
    for condition in chunk:
        # Unpack chunk
        muscles, moth, window_size, embed = condition
        ...
        key = f'muscle_{musclestring}_moth_{mothstring_no_underscore}_embed_{embed}_window_{window_size}_pid_{process_id}'
        print(key)
        # Load dataset
...
if __name__ == '__main__':
    ...
    window_size_range = np.logspace(np.log10(0.02), np.log10(0.5), 20)
    moth_range = ['2025-02-25', '2025-02-25_1']
    embed_range = np.array([4,8,12,16,20,24])
    main_iterator = product(
        [['ldlm'], ['rax'], ['lax', 'lba', 'lsa', 'rsa', 'rba', 'rax'], ['ldvm', 'ldlm', 'rdlm', 'rdvm']], 
        moth_range,
        window_size_range,
        embed_range
    )
    ...
    for key, single_result in results.items():
        ...
        with torch.no_grad():
            ...
            mi = - model(getattr(ds, 'Y'), getattr(ds, 'Z')).detach().cpu().numpy()
            del model
        mi_dict[key] = mi
        ...
        if (iteration_count % save_every_n_iterations == 0):
            try:
                save_dicts_to_h5([mi_dict, all_params], filename)
