...
if len(sys.argv) > 1: 
    ...
    filename = os.path.join(result_dir, datetime.today().strftime('%Y-%m-%d') + '_kinematics_neurons_' + machine + '_' + f'task_{task_id}' + '.h5')
# Otherwise just a single run
else:
    task_id = 0
    filename = os.path.join(result_dir, datetime.today().strftime('%Y-%m-%d') + '_kinematics_neurons_' + machine + '_' + '.h5')
...
def train_models_worker(chunk_with_id):
    ...
    for condition in chunk:
        ...
        ds = TimeWindowDatasetKinematics(os.path.join(data_dir, moth), window_size_range[wi], 
            ...
            only_flapping=True, angles_only=True, check_windows=True,
            use_ISI=False, use_phase=True
        )
        # Skip if there isn't enough data when checking windows
        if (ds.X.shape[0] * window_size_range[0]) < amount_of_time_threshold:
            continue
        ...
        for i,embed in enumerate(embed_dims):
            for j in range(embed_repeats):
                this_params = {**params, 
                    'X_dim': ds.X.shape[1] * ds.X.shape[2], 'Y_dim': ds.Z.shape[1] * ds.Z.shape[2],
                    'moth': moth,
                ...
                mi_test, train_id = train_model_no_eval(ds, this_params, X='X', Y='Z', verbose=False)
        ...
        synchronize()
        print(f'Condition {key} took {time.time() - tic}')
    return results
...
if __name__ == '__main__':
    ...
    for key, single_result in results.items():
        ...
        # Skip if there isn't enough data when checking windows
        ds = TimeWindowDatasetKinematics(os.path.join(data_dir, cond_params['moth']), window_size_range[0], 
                select_x=cond_params['neurons'], 
        ...
        for i,window_size in enumerate(window_size_range):
            ...
            with torch.no_grad():
                ...
                model.eval()
                precision_mi = precision_rounding(precision_levels, ds, model, X='X', Y='Z', early_stop=True, early_stop_threshold=0.5)
