...
def train_model_no_eval(dataset, params, 
    ...
    for epoch in range(epochs):
        ...
        # Test model at every epoch, deal with subsetting/window shifting
        with torch.no_grad():
            if model_name == "DSIB":
...
def train_model_with_eval(dataset, params, 
        device=device, 
        subset_times=None, 
        return_indices=False, 
        verbose=True,
    ...
    epochs = params['epochs']
    opt = torch.optim.Adam(model.parameters(), lr=params['learning_rate'], eps=params['eps'])
    estimates_mi_test, estimates_mi_train = [], []
    best_estimator_ts = float('-inf')  # Initialize with negative infinity
    no_improvement_count = 0
    ...
    for epoch in range(epochs):
        ...
        # Test and eval model at every epoch, deal with subsetting/window shifting
        with torch.no_grad():
            # Test
            if model_name == "DSIB":
                estimator_ts = - model(test_X, test_Y)
            ...
            estimates_mi_test.append(estimator_ts)
            # Eval 
            eval_X = getattr(dataset, X)[shuffled_train_indices, :, :]
            eval_Y = getattr(dataset, Y)[shuffled_train_indices, :, :]
            if model_name == "DSIB":
                estimator_tr = - model(eval_X, eval_Y)
            elif model_name == "DVSIB": # Get lossGout, that is the mi value
                _, _, estimator_tr = model(eval_X, eval_Y)
            estimator_tr = estimator_tr.detach().cpu().numpy()
    ...
    if return_indices:
        ...
        return np.array(estimates_mi_test), np.array(estimates_mi_train), train_id, train_indices
    else:
        return np.array(estimates_mi_test), np.array(estimates_mi_train), train_id
